    ...
    var Planner = {
      ...
      setDate: function(dateobj) {
        if (typeof(Planner.epoch) == 'undefined') {
          ...
          Planner.setEpoch(Planner.strToTime($parent.find(".epoch [name=epoch]").val()));
          return;
        }
      ...
      setEpoch: function(dateobj) {
        Planner.epoch = new Date(dateobj.getTime());
        $parent.find('.epoch [name=epoch]').val(Planner.timeToStr(Planner.epoch));
        $parent.find('.date').val($.datepicker.formatDate(Planner.dateFormat, Planner.epoch));

        $parent.find('.timeline .step').remove();
      ...
      fillTimeline: function(epoch) {
        ...
        if (typeof(epoch) == 'undefined' && $parent.find('.timeline .step').length > 0) {
          // adjust height
          epoch = Planner.strToTime($parent.find('.timeline .step:last').text());
        }

        var step=Planner.timeToHeight(Planner.step),
          cnt=Math.ceil($parent.find(".timeline").height() / step),
          i=0;
        cnt -= $parent.find(".timeline .step").length;

        $parent.find(".timeline form").height(step-1);
        for(i=0; i<cnt; i++) {
          var tstr = Planner.timeToStr(epoch, Planner.step);
          $parent.find(".timeline").append(
      ...
      newTrack: function (items) {
        ...
        $parent.find('.timeline').add('#planner-instructions').fadeIn();
      ...
      init: function(params) {
        ...
        $parent.find(".table .item").live('mouseover mouseout', function(event) {
        ...
        $parent.find(".table .item .field").attr('contenteditable', true)
        ...
        $parent.find(".table-wrap").resize(function() {
          var h=$(this).height();
          $parent.find(".timeline").height(h);

          Planner.fillTimeline();
        });

        $parent.find(".timeline .epoch [name=epoch]").keydown(function(e) {
